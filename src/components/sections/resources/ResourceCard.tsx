...
export const ResourceCard = ({ resource }: ResourceCardProps) => {
    return (
        // <Link to="/resources/$resourcesId" params={{ resourcesId: resource.id }} className="block h-full">
        <Card className="flex flex-col h-full bg-white overflow-hidden hover:shadow-xl transition-all duration-500 border border-slate-200 group rounded-2xl hover:-translate-y-1 p-0 gap-0">
            ...
            <div className="flex flex-col flex-grow p-6">
                ...
                <CardFooter className="p-0 mt-auto">
                    {/* <Button variant="ghost" className="w-full justify-between items-center h-12 text-md font-semibold bg-slate-50 text-slate-900 group-hover:bg-primary group-hover:text-white transition-all duration-300 rounded-xl">
                            {resource.buttonText}
