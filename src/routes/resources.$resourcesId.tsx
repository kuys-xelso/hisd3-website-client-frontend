...
function RouteComponent() {
    ...
    const relatedArticles = allArticles
        .filter((a: any) =>
            a.category?.name === article?.category?.name &&
            a.id !== article?.id
        )
        .slice(0, 2)
    ...
    if (loading) {
        return (
            <Section size="none" className='relative pt-20 pb-10 bg-gradient-to-b from-white/60 to-slate-50/60'>
                <Container>
                    <div className='flex flex-col gap-6 lg:w-3/4 animate-pulse'>
        ...
        return (
            <Section size="lg" className='text-center'>
                <Container>
                    ...
                    <p className='mb-8 text-muted-foreground'>We couldn't find the article you're looking for.</p>
                    <Link to='/resources'>
                            ...
                            {article.author?.username && (
                                <div className="flex items-center gap-3">
                                    <div className='flex flex-col'>
                                        <p className="text-base font-medium">{article.author.username}</p>
                    ...
                    <div className='relative z-0 flex flex-col lg:flex-row items-start gap-10 px-4 sm:px-6'>
                        ...
                        <div className='flex flex-col gap-8 lg:sticky lg:top-28 h-fit w-full lg:w-80 shrink-0'>
                            ...
                            <div className="flex flex-col gap-4">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 px-1 mb-2">More from HISD3</h3>
                                <div className="flex flex-col gap-4">
                                    {relatedArticles.map((item: any) => (
                                        ...
                                        >
                                            ...
                                            <Badge variant="secondary" className="w-fit text-[10px] uppercase font-bold">{item.category?.name}</Badge>
