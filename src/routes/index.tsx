...
function ApolloConnectionTest() {
  ...
  if (loading) return <div className="p-4 bg-blue-100 text-blue-800 rounded-md m-4">Testing connection to GraphQL...</div>;
  if (error) return <div className="p-4 bg-red-100 text-red-800 rounded-md m-4 border-red-500 border">Connection Error: {error.message}</div>;
  ...
  return (
    <div className="flex p-2 bg-green-100 text-green-800 rounded-md m-4 border-green-500 border">
      ...
      <p>Loaded {(data as any)?.__schema?.types?.length || 0} types from the backend schema.</p>
    </div>
  );
}
